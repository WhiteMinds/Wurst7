...
public final class AutoWoodcutterHack extends Hack implements UpdateListener, RenderListener {
  private final SliderSetting range = new SliderSetting("Range", 5, 1, 6, 0.05, ValueDisplay.DECIMAL);

  private final HashMap<String, Long> blockCooldownMap = new HashMap<>();
  ...
  public AutoWoodcutterHack() {
    super("AutoWoodcutter",
  ...
  public void onUpdate() {
    ...
    if (!WURST.getHax().freecamHack.isEnabled()) {
      blocksToHarvest = blocks.parallelStream().filter(this::shouldBeHarvested)
  ...
  private String getBlockKey(BlockPos pos) {
  ...
  private boolean shouldBeHarvested(BlockPos pos) {
    ...
    if (block == Blocks.OAK_SAPLING || block == Blocks.SPRUCE_SAPLING || block == Blocks.BIRCH_SAPLING
  ...
  private void registerPlants(List<BlockPos> blocks) {
    ...
    seeds.put(Blocks.OAK_SAPLING, Items.OAK_SAPLING);
    seeds.put(Blocks.SPRUCE_SAPLING, Items.SPRUCE_SAPLING);
    seeds.put(Blocks.BIRCH_SAPLING, Items.BIRCH_SAPLING);
    seeds.put(Blocks.JUNGLE_SAPLING, Items.JUNGLE_SAPLING);
    seeds.put(Blocks.ACACIA_SAPLING, Items.ACACIA_SAPLING);
    seeds.put(Blocks.DARK_OAK_SAPLING, Items.DARK_OAK_SAPLING);

    plants.putAll(blocks.parallelStream().filter(pos -> seeds.containsKey(BlockUtils.getBlock(pos)))
  ...
  private void placeBlockSimple(BlockPos pos) {
    ...
    for (int i = 0; i < sides.length; i++)
      hitVecs[i] = posVec.add(Vec3d.of(sides[i].getVector()).multiply(0.5));
    ...
    for (int i = 0; i < sides.length; i++) {
      ...
      if (MC.world.raycastBlock(eyesPos, hitVecs[i], neighbor, neighborShape, neighborState) != null)
        continue;
    ...
    blockCooldownMap.put(getBlockKey(pos), new Date().getTime());
  }
